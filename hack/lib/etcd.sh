...
kube::etcd::start() {
  ...
  kube::log::info "etcd --advertise-client-urls ${KUBE_INTEGRATION_ETCD_URL} --data-dir ${ETCD_DIR} --listen-client-urls http://${ETCD_HOST}:${ETCD_PORT} --experimental-watch-progress-notify-interval 1s --log-level=debug > \"${ETCD_LOGFILE}\" 2>/dev/null"
  etcd --advertise-client-urls "${KUBE_INTEGRATION_ETCD_URL}" --data-dir "${ETCD_DIR}" --listen-client-urls "${KUBE_INTEGRATION_ETCD_URL}" --experimental-watch-progress-notify-interval "1s" --log-level=debug 2> "${ETCD_LOGFILE}" >/dev/null &
  ETCD_PID=$!
