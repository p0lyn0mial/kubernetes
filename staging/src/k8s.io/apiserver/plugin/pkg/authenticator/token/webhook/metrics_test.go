	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			authnMetrics := AuthenticatorMetrics{
				RequestTotal:   fakeAuthnMetrics.RequestTotal,
				RequestLatency: fakeAuthnMetrics.RequestLatency,
			}
			...
			if fakeAuthnMetrics.totalCode != scenario.expectedRegisteredStatusCode {
				t.Errorf("incorrect status code recorded for RequestTotal method, expected = %v, got %v", scenario.expectedRegisteredStatusCode, fakeAuthnMetrics.totalCode)
			}
			if fakeAuthnMetrics.totalVerb != "POST" {
				t.Errorf("incorrect verb recorded for RequestTotal method, expected = POST, got %v", fakeAuthnMetrics.totalVerb)
			}

			if fakeAuthnMetrics.latencyCode != scenario.expectedRegisteredStatusCode {
				t.Errorf("incorrect status code recorded for RequestLatency method, expected = %v, got %v", scenario.expectedRegisteredStatusCode, fakeAuthnMetrics.latencyCode)
			}
			if fakeAuthnMetrics.latencyVerb != "POST" {
				t.Errorf("incorrect verb recorded for RequestLatency method, expected = POST, got %v", fakeAuthnMetrics.latencyVerb)
			}
...
func (f *fakeAuthenticatorMetrics) RequestTotal(_ context.Context, code string, verb string) {
	f.totalCode = code
	f.totalVerb = verb
}

func (f *fakeAuthenticatorMetrics) RequestLatency(_ context.Context, latency float64, code string, verb string) {
	f.latency = latency
	f.latencyCode = code
	f.latencyVerb = verb
}
