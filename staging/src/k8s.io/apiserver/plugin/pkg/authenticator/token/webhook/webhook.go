...
func NewFromInterface(tokenReview authenticationv1client.AuthenticationV1Interface, implicitAuds authenticator.Audiences, retryBackoff wait.Backoff, metrics AuthenticatorMetrics) (*WebhookTokenAuthenticator, error) {
	tokenReviewClient := &tokenReviewV1Client{tokenReview.RESTClient()}
	return newWithBackoff(tokenReviewClient, retryBackoff, implicitAuds, metrics)
}
...
func New(kubeConfigFile string, version string, implicitAuds authenticator.Audiences, retryBackoff wait.Backoff, customDial utilnet.DialFunc) (*WebhookTokenAuthenticator, error) {
	...
	return newWithBackoff(tokenReview, retryBackoff, implicitAuds, AuthenticatorMetrics{
...
func newWithBackoff(tokenReview tokenReviewer, retryBackoff wait.Backoff, implicitAuds authenticator.Audiences, metrics AuthenticatorMetrics) (*WebhookTokenAuthenticator, error) {
...
func (w *WebhookTokenAuthenticator) AuthenticateToken(ctx context.Context, token string) (*authenticator.Response, bool, error) {
	...
	if err := webhook.WithExponentialBackoff(ctx, w.retryBackoff, func() error {
		...
		if statusCode != 0 {
			w.metrics.RequestTotal(ctx, strconv.Itoa(statusCode), "POST")
			w.metrics.RequestLatency(ctx, latency.Seconds(), strconv.Itoa(statusCode), "POST")
			return tokenReviewErr
		}

		if tokenReviewErr != nil {
			w.metrics.RequestTotal(ctx, "<error>", "POST")
			w.metrics.RequestLatency(ctx, latency.Seconds(), "<error>", "POST")
		}
		return tokenReviewErr
