	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedAlternatives == nil {
				requireObjectsEqual(t, fmt.Sprintf("%s: incorrect pod", tt.name), tt.expectedOut, out)
			} else {
...
func RunTestUnconditionalDelete(ctx context.Context, t *testing.T, store storage.Interface) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			requireObjectsEqual(t, "incorrect pod:", tt.expectedObj, out)
		})
	}
}
...
func RunTestConditionalDelete(ctx context.Context, t *testing.T, store storage.Interface) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			out.ResourceVersion = storedObj.ResourceVersion
			requireObjectsEqual(t, "incorrect pod:", storedObj, out)
			obj := &example.Pod{ObjectMeta: metav1.ObjectMeta{Name: "foo", Namespace: "test-ns", UID: "A"}}
			key, storedObj = testPropagateStore(ctx, t, store, obj)
		})
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedAlternatives == nil {
				...
				requireObjectsEqual(t, "incorrect list pods", tt.expectedOut, out.Items)
			} else {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for j, wantPod := range tt.expectedOut {
				getPod := &out.Items[j]
				requireObjectsEqual(t, fmt.Sprintf("%s: incorrect pod", tt.name), wantPod, getPod)
			}
		})
	}
}
...
func RunTestListContinuation(ctx context.Context, t *testing.T, store storage.Interface, validation CallsValidation) {
	...
	requireObjectsEqual(t, "incorrect first page", []example.Pod{*preset[0].storedObj}, out.Items)
	if validation != nil {
		validation(t, 1, 1)
	}
	...
	requireObjectsEqual(t, "incorrect second page", []example.Pod{*preset[1].storedObj, *preset[2].storedObj}, out.Items)
	if validation != nil {
		validation(t, 0, 2)
	}
	...
	requireObjectsEqual(t, "incorrect second page", []example.Pod{*preset[1].storedObj}, out.Items)
	if validation != nil {
		validation(t, 1, 1)
	}
	...
	requireObjectsEqual(t, "incorrect third page", []example.Pod{*preset[2].storedObj}, out.Items)
	if validation != nil {
		validation(t, 1, 1)
	}
}
...
func RunTestListContinuationWithFilter(ctx context.Context, t *testing.T, store storage.Interface, validation CallsValidation) {
	...
	requireObjectsEqual(t, "incorrect first page", []example.Pod{*preset[0].storedObj, *preset[2].storedObj}, out.Items)
	if validation != nil {
		validation(t, 2, 3)
	}
	...
	requireObjectsEqual(t, "incorrect second page", []example.Pod{*preset[3].storedObj}, out.Items)
	if validation != nil {
		validation(t, 2, 1)
	}
}
...
func RunTestConsistentList(ctx context.Context, t *testing.T, store InterfaceWithPrefixTransformer) {
	...
	requireObjectsEqual(t, "incorrect lists", result1, result2)
	...
	requireObjectsEqual(t, "incorrect lists", result3, result4)
}
